...
System Prompt 為 LLM 的行為建立基礎。它們定義角色、專業領域、限制條件和輸出期望。
...
1. **明確具體**：模糊的角色會產生不一致的行為
2. **設定界限**：清楚定義模型應該和不應該做的事
3. **提供範例**：在 system prompt 中展示期望的行為
4. **徹底測試**：驗證 system prompt 在各種輸入下都能運作
5. **持續迭代**：根據實際使用模式進行優化
6. **版本控制**：追蹤 system prompt 的變更和效能

## 常見陷阱

- **過長**：過多的 system prompt 會浪費 token 並削弱焦點
- **過於模糊**：籠統的指示無法有效塑造行為
- **指令衝突**：矛盾的準則會讓模型混淆
- **過度限制**：太多規則會使回應僵化
- **格式規範不足**：缺少輸出結構會導致不一致
